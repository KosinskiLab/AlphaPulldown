...
def get_best_plddt(work_dir):
    ...
    best_plddt = pickle.load(
        open(os.path.join(work_dir, "result_{}.pkl".format(best_model)), "rb")
    )["plddt"]
...
def score_complex(path_coords, path_CB_inds, path_plddt):
    ...
    for i in chain_inds:
        ...
        for int_i in np.setdiff1d(chain_inds, i):
            ...
            int_chain_CB_coords = np.array(path_coords[int_chain])[
                path_CB_inds[int_chain]
            ]
            ...
            if contacts.shape[0] > 0:
                av_if_plDDT = np.concatenate(
                    (chain_plddt[contacts[:, 0]], int_chain_plddt[contacts[:, 1]])
                ).mean()
